...
export default function ItemDetailModal({ 
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center p-0 md:p-6 bg-white/90 backdrop-blur-3xl animate-fade-in">
      <div className="bg-card border border-border w-full max-w-2xl rounded-t-[4rem] md:rounded-[4rem] overflow-hidden relative max-h-[95vh] flex flex-col shadow-[0_0_150px_rgba(0,0,0,0.1)]">
        
        {/* Close Button */}
        <button 
          onClick={onClose} 
          className="absolute top-8 right-8 z-50 w-14 h-14 flex items-center justify-center bg-white/60 border border-border backdrop-blur-xl rounded-full text-4xl hover:bg-primary hover:text-white transition-all text-secondary"
        >
          ×
        </button>
          ...
          <div className="space-y-10">
            <div className="flex flex-col md:flex-row justify-between items-start gap-8">
              <div className="space-y-4 max-w-md">
                 ...
                 <h2 className="text-6xl md:text-7xl font-black tracking-tighter serif italic leading-none text-secondary">{itemName}</h2>
              </div>
              <span className="text-4xl font-black text-primary tracking-tighter leading-none pt-2">{item.price} DH</span>
            </div>
            
            <p className="text-xl md:text-2xl font-light text-foreground/40 italic leading-relaxed serif">
              "{itemDesc}"
            </p>
            
            <div className="w-full h-px bg-secondary/5" />
                      ...
                      <button
                        ...
                        className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all duration-500 ${
                          isSelected 
                            ...
                            : 'border-secondary/10 bg-secondary/5 hover:border-secondary/30 text-secondary/60'
                        }`}
                      >
                        <span className="text-xs font-black uppercase tracking-widest">{optLabel}</span>
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                          isSelected ? 'border-primary bg-primary scale-110' : 'border-secondary/10'
                        }`}>
        ...
        <div className="p-10 pt-0 md:p-16 md:pt-0">
          <button 
            onClick={handleAdd} 
            className="group relative w-full py-8 overflow-hidden rounded-full bg-[#EC3F28] text-white font-black uppercase tracking-[0.4em] text-[10px] transition-all hover:scale-[1.02] active:scale-95 shadow-[0_20px_60px_rgba(236,63,40,0.2)] border border-primary/20"
          >
